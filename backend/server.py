...
# ─────────────────────────────────────────────────────────────────────────────
# CONSUMPTION RATES — purchase-frequency based stock estimation
# ─────────────────────────────────────────────────────────────────────────────
import time as _time_mod

_reorder_cache: dict = {"text": None, "ts": 0}
_REORDER_CACHE_TTL = 60 * 60  # 1 hour
...
async def consumption_rates(user=Depends(get_current_user)):
    ...
    sorted_rates = sorted(
        rates.values(),
        key=lambda x: (STATUS_ORDER.get(x["status"], 9), x.get("est_days_remaining") or 999)
    )
    return {"rates": sorted_rates, "total": len(sorted_rates)}


@api.get("/smart-reorder")
async def smart_reorder(user=Depends(require_roles("admin", "viewer"))):
    global _reorder_cache
    from services.consumption import get_consumption_rates

    if _reorder_cache["text"] and (_time_mod.time() - _reorder_cache["ts"]) < _REORDER_CACHE_TTL:
    ...
    try:
        rates = await get_consumption_rates(db)
        actionable = [r for r in rates.values()
                      if r["status"] in ("urgent", "soon", "overdue")
                      and r["confidence"] != "insufficient"]
        ok_items = [r for r in rates.values()
                    if r["status"] == "ok" and r.get("est_days_remaining")]

        if not actionable and not ok_items:
            return {"insight": "Not enough purchase history yet. Keep recording purchases!", "cached": False}
        ...
        resp.raise_for_status()
        insight = resp.json()["choices"][0]["message"]["content"].strip()
        _reorder_cache = {"text": insight, "ts": _time_mod.time()}
        return {"insight": insight, "cached": False}
...
@api.post("/smart-reorder/refresh")
async def refresh_smart_reorder(user=Depends(require_roles("admin"))):
    global _reorder_cache
